...
String sql = "select date_format(savetime,'%Y-%m-%d') as days ,count(*) as count from ddinfo where fkstatus='已付款' group by days";
	ArrayList<HashMap> list = (ArrayList<HashMap>)dao.select(sql);
%>
<div class="row-fluid">
    <div class="block">
        <a href="#page-stats" class="block-heading" data-toggle="collapse">订单量分析</a>
        <div id="page-stats" class="block-body collapse in">

            <div class="stat-widget-container">
                <div id="chartdiv" align="center"> FusionCharts. </div>
  	<script language="JavaScript" src="/jiu_buy_online/admin/FusionCharts.js"></script>
  	 <script type="text/javascript">
		var chart = new FusionCharts("/jiu_buy_online/admin/FCF_Line.swf", "ChartId", "900", "300");
...
//String sql = "select outtime,count(*) as sl from cwtocar where outtime is not null group by outtime  ";
		...
		for(HashMap m:list){
		 %>
		  "   <set name='<%=m.get("days")%>' value='<%=m.get("count")%>'/> "+
		<%}%>
		  "   </graph>"; 
