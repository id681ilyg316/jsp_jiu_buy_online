...
<html lang="en">
  ...
  <body class="">
    ...
    <%	 CommDAO dao = new CommDAO(); 
	 ...
	 String url = "/jiu_buy_online/admin/goodskc.jsp?1=1";
			...
			<%}%>
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/goodskc.jsp?f=f" class="form-inline">
   ...
   ArrayList<HashMap> list=(ArrayList)bean.getCollection();
   	for(HashMap map:list){
        ...
        <tr>
          ...
          <td>
          	<%=Info.getkc(map.get("id").toString()) %>
		    <%if(Info.getkc(map.get("id").toString())==0){ %>
		    <font color="red">(该商品库存不足，请补仓)</font>
		    <%} %>
          </td>
          <td>
              <a href="admin/goodskcadd.jsp?gid=<%=map.get("id") %>" title="入库"><i class="icon-plus"></i></a>
          </td>
        </tr>
        <%} %>
      </tbody>
