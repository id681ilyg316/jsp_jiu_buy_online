...
for(HashMap map:list){
	ArrayList<HashMap> numlist = (ArrayList<HashMap>)dao.select("select * from dddetail where goodid="+map.get("id"));
	int a = 0;
	for(HashMap mm:numlist){
		a+=Integer.valueOf(mm.get("sl").toString());
	}
	map.put("sl",a);
}
%>
    ...
    <div class="block">
  	...
  	<script language="JavaScript" src="/jiu_buy_online/admin/FusionCharts.js"></script>
  	 <script type="text/javascript">
		var chart = new FusionCharts("/jiu_buy_online/admin/FCF_Column3D.swf", "ChartId", "900", "300");
...
//String sql = "select outtime,count(*) as sl from cwtocar where outtime is not null group by outtime  ";
		...
		for(HashMap m:list){
		 %>
		  "   <set name='<%=m.get("goodname")%>' value='<%=m.get("sl")%>'/> "+
		<%}%>
		  "   </graph>"; 
