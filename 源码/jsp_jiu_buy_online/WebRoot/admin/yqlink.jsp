...
<html lang="en">
  ...
  <body class="">
    ...
    <%	 CommDAO dao = new CommDAO(); 
     String sql = "select * from yqlink where 1=1 ";
	 String url = "/jiu_buy_online/admin/yqlink.jsp?1=1";
			...
			<%}%>
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/yqlink.jsp?f=f" class="form-inline">
...
<div class="well">
   ...
   if(did!=null)
   {
	   dao.commOper("delete from  yqlink  where id="+did);
   }
   ...
   ArrayList<HashMap> list=(ArrayList)bean.getCollection();
   	for(HashMap map:list){
	    %>
        <tr>
          <td><%=map.get("linkname") %></td>
          <td><%=map.get("linkurl") %></td>
          <td>
              <a href="admin/yqlinkedit.jsp?id=<%=map.get("id") %>"><i class="icon-pencil"></i></a>
              <a href="admin/yqlink.jsp?did=<%=map.get("id") %>" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        <%} %>
      </tbody>
